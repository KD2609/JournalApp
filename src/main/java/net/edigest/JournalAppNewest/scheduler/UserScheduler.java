...
import net.edigest.JournalAppNewest.Enums.Sentiment;
...
public class UserScheduler {
    ...
    public void fetchUserAndSaMail(){
        ...
        for(User user : users) {
            ...
            Map<Sentiment,Integer> setimentCounts = new HashMap<>();

            for(Sentiment sentiment : sentiments) {
                if(sentiment != null) {
                    setimentCounts.put(sentiment,setimentCounts.getOrDefault(sentiment,0) + 1);
                }
            }
            ...
            int maxCount = 0;
            for(Map.Entry<Sentiment,Integer> entry : setimentCounts.entrySet()) {
